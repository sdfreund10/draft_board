...
constructor(props) {
  ...
  leftTeamTable() {
    ...
    for(var i = 0; i < this.props.teams.length; i += 2 ) {
      ...
      evenTeams.push(this.teamRow(team));
    }
    return(evenTeams);
  }
  ...
  rightTeamTable() {
    ...
    for(var i = 1; i < this.props.teams.length; i += 2 ) {
      var team = this.props.teams[i];
      oddTeams.push(this.teamRow(team));
    }
    return(oddTeams);
  }
  ...
  teamRow(team) {
    return(
      <tr key={team.pick}>
        <td>{team.pick}</td>
        <td><input value={team.name} onChange={(e) => this.changeName(e, team)}/></td>
        <td><button type='button' className='btn priamary btn-sm' onClick={(e) => this.props.removeTeam(e, team)}>X</button></td>
      </tr>
    );
  }
